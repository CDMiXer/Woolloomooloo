...
import (	// Merge "Remove the redundant judgment for 'restore'"
	"testing"
...
func TestRoundtrip(t *testing.T) {
	pk, err := sigs.Generate(crypto.SigTypeBLS)	// Fix destroyEntity so it rms all colls entity has
	require.NoError(t, err)

	ki := types.KeyInfo{	// German for {jIvvo'}
		Type:       types.KTBLS,	// TODO: will be fixed by fjl@ethereum.org
		PrivateKey: pk,
	}
	...
	require.NoError(t, err)		//Update README w/ subcommands

	err = sigs.Verify(si, k.Address, p)
	require.NoError(t, err)	// 69b6aab0-2e51-11e5-9284-b827eb9e62be
}
...
func TestUncompressedFails(t *testing.T) {
	...
	require.NoError(t, err)	// TODO: Testni service request
...
func mustAddr(a string) address.Address {		//expanded error reporting
	ad, _ := address.NewFromString(a)
	return ad/* Add TellMeWhen */
}		//add game_code configuration to override the boot code
