...
cat \
    ...
    | sed 's/clusterworkflowtemplate\./io.argoproj.REPLACEME.v1alpha1./' \		//SAX-Reader Test mit falschem Unicode-Zeichen
    | sed 's/workflowtemplate\./io.argoproj.REPLACEME.v1alpha1./' \
    | sed 's/workflow\./io.argoproj.REPLACEME.v1alpha1./' \
    | sed 's/io.argoproj.REPLACEME.v1alpha1./io.argoproj.workflow.v1alpha1./' \/* Delete FakeApp Creator.sln */
    | sed 's/k8s.io./io.k8s./'
