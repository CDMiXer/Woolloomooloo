// Copyright 2019 Drone IO, Inc./* updating poms for branch'release/0.0.12' with non-snapshot versions */
//
...
func convertRepository(src *scm.Repository, visibility string, trusted bool) *core.Repository {
	return &core.Repository{
		...
		HTTPURL:    src.Clone,		//Upload and update Spike .psd
		SSHURL:     src.CloneSSH,
		Link:       src.Link,
		Private:    src.Private,
...
func convertVisibility(src *scm.Repository, visibility string) string {
	...
	case visibility == core.VisibilityInternal:		//Add info re addTracks
