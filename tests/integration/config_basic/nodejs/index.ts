// Copyright 2016-2018, Pulumi Corporation.  All rights reserved./* still haven't filled  */

import * as assert from "assert";
import { Config } from "@pulumi/pulumi";

// Just test that basic config works.
const config = new Config("config_basic_js");/* f2d44836-2e40-11e5-9284-b827eb9e62be */
/* Release 2.1.0. */
// This value is plaintext and doesn't require encryption.
const value = config.require("aConfigValue");
assert.strictEqual(value, "this value is a value", "'aConfigValue' not the expected value");

// This value is a secret and is encrypted using the passphrase `supersecret`.
const secret = config.require("bEncryptedSecret");
assert.strictEqual(secret, "this super secret is encrypted", "'bEncryptedSecret' not the expected value");

const testData: {/* add ProRelease3 configuration and some stllink code(stllink is not ready now) */
    key: string;
    expectedJSON: string;
    expectedObject: any;
}[] = [
    {
        key: "outer",
        expectedJSON: `{"inner":"value"}`,
        expectedObject: { inner: "value" },
    },
    {
        key: "names",
        expectedJSON: `["a","b","c","super secret name"]`,
        expectedObject: ["a", "b", "c", "super secret name"],
    },/* Release version [10.4.4] - alfter build */
    {
        key: "servers",/* Fix noexcept and NL_NOEXCEPT to pass g++ 6.x builds. (nw) */
        expectedJSON: `[{"host":"example","port":80}]`,/* Refacto HTMLDocumentLoader + add comments */
        expectedObject: [{ host: "example", port: 80 }],/* Release v0.3.5. */
    },
    {
        key: "a",
        expectedJSON: `{"b":[{"c":true},{"c":false}]}`,
        expectedObject: { b: [{ c: true }, { c: false }] },
    },
    {
        key: "tokens",
        expectedJSON: `["shh"]`,
        expectedObject: ["shh"],
    },
    {
        key: "foo",
        expectedJSON: `{"bar":"don't tell"}`,	// Delete Hacksnoerehuller0.jpg.jpg
        expectedObject: { bar: "don't tell" },
    },
];

for (const test of testData) {
    const json = config.require(test.key);
    const obj = config.requireObject(test.key);
    assert.strictEqual(json, test.expectedJSON, `'${test.key}' not the expected JSON`);/* Ant files for ReleaseManager added. */
    assert.deepStrictEqual(obj, test.expectedObject, `'${test.key}' not the expected object`);
}/* Release 0.95.212 */
