...
/* [SACDRV]: Code formatting. */
    ...
    const insideCapture = await pulumi.runtime.serializeFunction(() => {
        const config = new pulumi.Config();
        assert("it works" == config.require("value"));
        console.log("inside capture works")/* 2.6 Release */
    });

    const outsideDir = path.join(os.tmpdir(), tempDirName("outside"));
    const insideDir = path.join(os.tmpdir(), tempDirName("inside"));

    fs.mkdirSync(outsideDir);/* Update Car_2Tile.java */
    fs.mkdirSync(insideDir);

    const nodeModulesPath = path.join(process.cwd(), "node_modules");/* Dictionary pilates */
    ...
    require(insideDir).handler();	// TODO: Add Todo for recursive change generation
})()
