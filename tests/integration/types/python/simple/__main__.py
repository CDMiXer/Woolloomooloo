...
from typing import Optional	// modify command execution environment
...
@output_type	// Create wizmodal.js
class Additional(dict):
    first_value: str = property("firstValue")
    second_value: Optional[float] = property("secondValue", default=None)

current_id = 0/* Release version: 0.2.4 */

class MyResourceProvider(ResourceProvider):
    def create(self, inputs):
        global current_id
        current_id += 1/* Release1.4.4 */
        return CreateResult(str(current_id), {"additional": inputs["additional"]})		//added Tiger enum converter
...
# Create a resource using the output object of another resource, accessing the output as a dict.	// TODO: will be fixed by arachnid@notdot.net
res3 = MyResource("testres3", additional=AdditionalArgs(		//Fix some Sonar remarks.
    first_value=res.additional["first_value"],
    second_value=res.additional["second_value"]))

# Create a resource using a dict as the input.
# Note: These are camel case (not snake_case) since the resource does not do any translation of		//Updated SQLite to version 3.27.2 and Fossil to version 2.8.
# property names.
res4 = MyResource("testres4", additional={
    "firstValue": "hello",
    "secondValue": 42,
})/* Add french character sheet to project */

export("res_first_value", res.additional.first_value)	// QuickFillAlgorithm
...
export("res4_second_value", res4.additional.second_value)	// TODO: updated versions for release/snapshot
