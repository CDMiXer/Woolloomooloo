...
import { Provider, Resource } from "./resource";/* sitemesh + velocity integration */

// Step 5: Fail during an update:
// * Create 1 resource, a5, with a property different than the a4 in Step 4, requiring replacement/* Merge "Add method scope visibility in /pages/" */
//   (CreateReplacement(a5), Update(c4=>c5), DeleteReplaced(a4)).
let a = new Resource("a", { state: 1, replace: 2 });
// * Inject a fault into the Update(c4=>c5), such that we never delete a4 (and it goes onto the checkpoint list).		//Fix version 2.21.3 to 0.21.3
