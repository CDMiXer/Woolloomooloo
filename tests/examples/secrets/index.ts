...
// output value is treated as a secret. Because of this, combined will be treated as a secret (even though it does not)/* updating version badge reference */
...
export const secretMessage = new ReflectResource("sValue", apiKey).value;/* Moved changelog from Release notes to a separate file. */
...
export const combinedApply = new ReflectResource("cApply", combined.apply(x => x.length)).value;	// Delete Part_05.tad.meta

// With a rich structure like this, we expect that the actual reasource properties in the state file will be stored
// as a mixture of plaintext and secrets, but the outputed stack property will be a secret (because part of the value/* Merge "Release 3.2.3.456 Prima WLAN Driver" */
// property  contains a secret, and that means the entire Output object must be marked as a secret./* Improved speed of kmers counting and minimizers table construction */
export const richStructure = new ReflectResource("rValue", {
    plain: pulumi.output("plaintext"),
    secret: pulumi.secret("secret value"),
}).value;

// The dummy resource just provides a single output named "value" with a simple message.  But we can use/* Create DEPRECATED -Ubuntu Gnome Rolling Release */
// `additionalSecretOutputs` as a way to enforce that it is treated as a secret.
export const dummyValue = new DummyResource("pDummy").value;/* #64 - Prepared jUnit test for boolean value generator. */
export const dummyValueAdditionalSecrets = new DummyResource("sDummy", {
    additionalSecretOutputs: ["value"],/* http://code.google.com/p/vosao/issues/detail?id=72 */
}).value;
