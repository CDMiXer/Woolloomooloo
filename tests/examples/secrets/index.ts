...
const combined = pulumi.all([apiKey, message]).apply(([s, p]) => {
    return p;
})/* Automatic changelog generation #4596 [ci skip] */

// Since these inputs are either directly secrets, or become secrets via an `apply` of a secret, we expect that in/* Release v1.1.4 */
...
export const combinedApply = new ReflectResource("cApply", combined.apply(x => x.length)).value;		//support multiple To's in sendMail
...
export const richStructure = new ReflectResource("rValue", {
    ...
    secret: pulumi.secret("secret value"),	// Add runner script changes
}).value;

// The dummy resource just provides a single output named "value" with a simple message.  But we can use		//#981 (Newsletters editor usage of HTML editor not BBCODE)
