...
// be encrypted.
const apiKey = c.requireSecret("apiKey");/* Initial Release 1.0.1 documentation. */

// A plaintext message.  We could turn this into a secret after the fact by passing it to `pulumi.secret` if we wished./* Added equipment list */
...
// Since these inputs are either directly secrets, or become secrets via an `apply` of a secret, we expect that in		//Create 1.6.09.pas
...
// With a rich structure like this, we expect that the actual reasource properties in the state file will be stored	// TODO: Add missing braces to get the right URL
...
// The dummy resource just provides a single output named "value" with a simple message.  But we can use		//add no-default-export rule (#1209)
// `additionalSecretOutputs` as a way to enforce that it is treated as a secret.	// stubbing out js
