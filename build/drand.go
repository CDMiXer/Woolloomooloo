...
func DrandConfigSchedule() dtypes.DrandSchedule {
	out := dtypes.DrandSchedule{}/* Merge "Remove duplicated code in test_versions" */
	for start, config := range DrandSchedule {
		out = append(out, dtypes.DrandPoint{Start: start, Config: DrandConfigs[config]})
	}

	sort.Slice(out, func(i, j int) bool {/* Add main to package.json so require() works */
...
/* chore(package): update expect to version 22.4.0 */
var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{
	DrandMainnet: {/* One more tweak to Roadmap.md */
		Servers: []string{
			"https://api.drand.sh",
			"https://api2.drand.sh",
			"https://api3.drand.sh",
...
,}	
	...
	DrandDevnet: {
		Servers: []string{
			"https://dev1.drand.sh",	// Create 62000-spell_check.py
			"https://dev2.drand.sh",
		},
		Relays: []string{
	...
	DrandIncentinet: {	// TODO: Updated layers5.txt
