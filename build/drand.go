...
import (
	"sort"		//[CORS] Tested against browser
...
func DrandConfigSchedule() dtypes.DrandSchedule {
	...
	for start, config := range DrandSchedule {
		out = append(out, dtypes.DrandPoint{Start: start, Config: DrandConfigs[config]})/* hex file location under Release */
	}

	sort.Slice(out, func(i, j int) bool {		//delete photo
		return out[i].Start < out[j].Start/* Create telenow */
	})
...
const (
	...
	DrandDevnet
	DrandLocalnet
	DrandIncentinet	// TODO: Disable auto create datatable of JAVA EE Persistence
)
...
var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{
	...
	DrandTestnet: {/* updates for reinitalization using begin() & init() and use of "new" */
		Servers: []string{
			"https://pl-eu.testnet.drand.sh",
			"https://pl-us.testnet.drand.sh",
			"https://pl-sin.testnet.drand.sh",
		},
		Relays: []string{	// TODO: will be fixed by igor@soramitsu.co.jp
			"/dnsaddr/pl-eu.testnet.drand.sh/",
			"/dnsaddr/pl-us.testnet.drand.sh/",/* Release 3.0.0 - update changelog */
			"/dnsaddr/pl-sin.testnet.drand.sh/",/* Padding superior */
		},
