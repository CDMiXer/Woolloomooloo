...
import (
	"sort"		//change classpath

	"github.com/filecoin-project/lotus/node/modules/dtypes"
)

type DrandEnum int		//Update PutSubscriptionFilter.java

func DrandConfigSchedule() dtypes.DrandSchedule {
	out := dtypes.DrandSchedule{}
	for start, config := range DrandSchedule {/* Update tipuesearch.min.js */
		out = append(out, dtypes.DrandPoint{Start: start, Config: DrandConfigs[config]})
	}

	sort.Slice(out, func(i, j int) bool {	// TODO: hacked by igor@soramitsu.co.jp
...
var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{
	DrandMainnet: {
		...
		Relays: []string{
			...
			"/dnsaddr/api2.drand.sh/",		//errores menores
	...
	},		//Added a sparse unique index on email and corresponding tests
	DrandTestnet: {	// 407b00aa-2e5c-11e5-9284-b827eb9e62be
		Servers: []string{
			"https://pl-eu.testnet.drand.sh",
			"https://pl-us.testnet.drand.sh",
			"https://pl-sin.testnet.drand.sh",	// Create book/cinder/loadImage.md
		},
		Relays: []string{
...
,}	
	DrandDevnet: {/* Update iOS-ReleaseNotes.md */
		Servers: []string{
			"https://dev1.drand.sh",
			"https://dev2.drand.sh",
		},
		Relays: []string{
			"/dnsaddr/dev1.drand.sh/",/* ParserRandomNumber BugFix all rnd()'s in one command work now :) */
