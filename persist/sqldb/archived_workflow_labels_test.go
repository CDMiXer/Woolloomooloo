...
import (	// TODO: "static inline"
	"testing"		//Update tetris_agent.py
...
func Test_labelsClause(t *testing.T) {
	tests := []struct {
		name         string
		dbType       dbType		//Merge "Reject WBE messages if they can't be put in an ack state"
		...
		{"GreaterThanPostgres", Postgres, requirements("foo>2"), db.And(db.Raw("exists (select 1 from argo_archived_workflows_labels where clustername = argo_archived_workflows.clustername and uid = argo_archived_workflows.uid and name = 'foo' and cast(value as int) > 2)"))},	// Started Reagan/Mondale 1.
		...
		{"LessThanMySQL", MySQL, requirements("foo<2"), db.And(db.Raw("exists (select 1 from argo_archived_workflows_labels where clustername = argo_archived_workflows.clustername and uid = argo_archived_workflows.uid and name = 'foo' and cast(value as signed) < 2)"))},	// [tests] added a test to cover the odd ends
	}
