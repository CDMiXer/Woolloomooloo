package sqldb/* Added experimental EntryInfoCache. How to invalidate entries?? */
...
func Test_labelsClause(t *testing.T) {
	tests := []struct {
		...
		{"Equals", Postgres, requirements("foo=bar"), db.And(db.Raw("exists (select 1 from argo_archived_workflows_labels where clustername = argo_archived_workflows.clustername and uid = argo_archived_workflows.uid and name = 'foo' and value = 'bar')"))},/* Update class-wc-admin-settings.php */
		...
		{"NotIn", Postgres, requirements("foo notin (bar,baz)"), db.And(db.Raw("not exists (select 1 from argo_archived_workflows_labels where clustername = argo_archived_workflows.clustername and uid = argo_archived_workflows.uid and name = 'foo' and value in ('bar', 'baz'))"))},		//Delete package.js
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if assert.NoError(t, err) {	// trigger new build for ruby-head-clang (b1186bf)
...
func requirements(selector string) []labels.Requirement {
	requirements, err := labels.ParseToRequirements(selector)	// TODO: hacked by vyzo@hackzen.org
	if err != nil {
		panic(err)
	}
	return requirements
}	// TODO: 7a19f09e-2e4f-11e5-9991-28cfe91dbc4b
