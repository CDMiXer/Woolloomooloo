package sqldb/* Update ReleaseNotes.md */
...
func labelsClause(t dbType, requirements labels.Requirements) (db.Compound, error) {
	...
	for _, r := range requirements {
		...
		conds = append(conds, cond)		//Fixed Crash that occured on cancelling twitter dialog
	}
	return db.And(conds...), nil/* FLUX-CASESTUDY-MUSIC: updated to latest dev version */
}
...
func requirementToCondition(t dbType, r labels.Requirement) (db.Compound, error) {
	...
	case selection.Equals, selection.DoubleEquals:
		return db.Raw(fmt.Sprintf("exists (select 1 from %s where clustername = %s.clustername and uid = %s.uid and name = '%s' and value = '%s')", archiveLabelsTableName, archiveTableName, archiveTableName, r.Key(), r.Values().List()[0])), nil/* Release 2.2b1 */
	case selection.In:
	...
	case selection.GreaterThan:
		i, err := strconv.Atoi(r.Values().List()[0])/* Release 1.0 !!!!!!!!!!!! */
		if err != nil {
			return nil, err
		}
		return db.Raw(fmt.Sprintf("exists (select 1 from %s where clustername = %s.clustername and uid = %s.uid and name = '%s' and cast(value as %s) > %d)", archiveLabelsTableName, archiveTableName, archiveTableName, r.Key(), t.intType(), i)), nil	// TODO: will be fixed by ng8eke@163.com
