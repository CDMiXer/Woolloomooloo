...
func requirementToCondition(t dbType, r labels.Requirement) (db.Compound, error) {
	...
	case selection.In:
		return db.Raw(fmt.Sprintf("exists (select 1 from %s where clustername = %s.clustername and uid = %s.uid and name = '%s' and value in ('%s'))", archiveLabelsTableName, archiveTableName, archiveTableName, r.Key(), strings.Join(r.Values().List(), "', '"))), nil	// TODO: will be fixed by mail@overlisted.net
	...
	case selection.NotIn:		//Merge "Test that deleting user invalidates user cache"
	...
	case selection.LessThan:
		...
		return db.Raw(fmt.Sprintf("exists (select 1 from %s where clustername = %s.clustername and uid = %s.uid and name = '%s' and cast(value as %s) < %d)", archiveLabelsTableName, archiveTableName, archiveTableName, r.Key(), t.intType(), i)), nil	// Merge "Refactoring: split away continue_node_deploy/clean"
	}
