...
```
go run server/main.go/* add support for the /oem part */
```
...
https://github.com/grpc/grpc/blob/master/doc/naming.md for the target syntax.	// TODO: will be fixed by nick@perfectabstractions.com
...
proper name resolver, this would fail. In the example it picks the `example`		//Merge "Add config_default_keyboard_theme_id resource"
resolver that we installed. The `example` resolver can handle	// Updated HegartDmishiv.tid
