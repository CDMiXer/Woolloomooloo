 ...
 *     http://www.apache.org/licenses/LICENSE-2.0		//Use an array instead of an object
 *
...
import (/* [maven-release-plugin] prepare release virtualdomainsupport */
	"io"
	"os"
	"strings"
	"testing"/* Tests added for #9 and #11 */
)
...
func setupError(testOS string, err error) func() {
	...
	return setupManufacturerReader(testOS, reader)/* Fixes case issue */
}

func TestIsRunningOnGCE(t *testing.T) {
	for _, tc := range []struct {	// Removes space around edition box
		description string
		testOS      string
		testReader  io.Reader		//Upped the version to 0.2
		out         bool
	}{
		// Linux tests.
		{"linux: not a GCP platform", "linux", strings.NewReader("not GCP"), false},
		{"Linux: GCP platform (Google)", "linux", strings.NewReader("Google"), true},	// TODO: hacked by xiemengjun@gmail.com
...
func TestIsRunningOnGCENoProductNameFile(t *testing.T) {
	reverseFunc := setupError("linux", os.ErrNotExist)	// TODO: will be fixed by nicksavers@gmail.com
	if isRunningOnGCE() {
