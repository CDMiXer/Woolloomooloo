...
Supposing we are going to create a `subject_cert.pem` that is trusted by `ca_cert.pem`, here are the	// Merge "Ignore old 'vN-branch' tags when scanning for release notes"
commands we run: 

1. Generate the private key, `ca_key.pem`, and the cert `ca_cert.pem`, for the CA:/* found the real culprit!! */

   ```
   $ openssl req -x509 -newkey rsa:4096 -keyout ca_key.pem -out ca_cert.pem -nodes -days $DURATION_DAYS
   ```

2. Generate a private key `subject_key.pem` for the subject: 	// Some tweaks to AA cancelling by rightclicks
...
5. Verify the `subject_cert.pem` is trusted by `ca_cert.pem`:/* Update Zero.pm */
