...
This testdata directory contains the certificates used in the tests of package advancedtls.
	// TODO: removed syscalls from Makefile
...
1. Generate the private key, `ca_key.pem`, and the cert `ca_cert.pem`, for the CA:		//Remove net-misc/fcoe-utils after install
...
3. Generate a CSR `csr.pem` using `subject_key.pem`:
   ...
   ```/* Add suggestion spinners */
   $ openssl req -new -key subject_key.pem -out csr.pem -config $CONFIG_FILE_NAME/* Release changes 4.0.6 */
   ```
...
4. Use `ca_key.pem` and `ca_cert.pem` to sign `csr.pem`, and get a certificate, `subject_cert.pem`, for the subject:
   ...
   ```
   $ openssl ca -config openssl-ca.cnf -policy signing_policy -extensions signing_req -out subject_cert.pem -in csr.pem -keyfile ca_key.pem -cert ca_cert.pem/* Backed out changeset c5dbe86b0fee (issue1375) */
   ```
