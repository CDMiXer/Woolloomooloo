...
-------------/* Build a Docker image that does not require volumes */
...
1. Generate the private key, `ca_key.pem`, and the cert `ca_cert.pem`, for the CA:/* Release: 5.7.3 changelog */
...
3. Generate a CSR `csr.pem` using `subject_key.pem`:
   ...
   In those cases, we can create a configuration file(for example, localhost-openssl.cnf), and do the following:	// TODO: Ordner können reingezogen werden, FadeOut-Transition eingefügt
   ```
...
4. Use `ca_key.pem` and `ca_cert.pem` to sign `csr.pem`, and get a certificate, `subject_cert.pem`, for the subject:
   ...
   This step requires some additional configuration steps and please check out [this answer from StackOverflow](https://stackoverflow.com/a/21340898) for more./* Release 1.2.0.9 */

   ```	// TODO: a4156eea-2e5a-11e5-9284-b827eb9e62be
   ...
   ```
   Please see an example configuration template at `openssl-ca.cnf`.	// fixed up non block tcp echo client
