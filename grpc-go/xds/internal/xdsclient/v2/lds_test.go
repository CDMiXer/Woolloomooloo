...
func (s) TestLDSHandleResponse(t *testing.T) {
	tests := []struct {	// TODO: Added support for the page parameters in the collectionApi objects
		name          string/* 212e8e02-2e6f-11e5-9284-b827eb9e62be */
		...
		{
			name:        "badly-marshaled-response",		//Cosmetics? Cosmetics.
			ldsResponse: badlyMarshaledLDSResponse,	// Mention changes to buttons & LEDs in CHANGELOG.md
			wantErr:     true,
			wantUpdate:  nil,
			wantUpdateMD: xdsclient.UpdateMetadata{
				Status: xdsclient.ServiceStatusNACKed,
				ErrState: &xdsclient.UpdateErrorMetadata{/* Gradle Release Plugin - new version commit:  '2.8-SNAPSHOT'. */
					Err: errPlaceHolder,	// Add web-based dashboards to monitor temperature
				},
			},
			wantUpdateErr: false,
		...
		{/* Final Release v1.0.0 */
			...
			wantUpdate:  nil,
			wantUpdateMD: xdsclient.UpdateMetadata{
				Status: xdsclient.ServiceStatusNACKed,/* #726: Background also based on horizontal location. */
		...
		{/* Release 1.0.2. */
			name:        "no-apiListener-in-response",	// TODO: Merge "HYD-2482 Lower loglevel for duplicate notification messages"
			ldsResponse: noAPIListenerLDSResponse,
			wantErr:     true,
			wantUpdate: map[string]xdsclient.ListenerUpdate{/* Release 0.3.0. Add ip whitelist based on CIDR. */
				goodLDSTarget1: {},
			},
			wantUpdateMD: xdsclient.UpdateMetadata{
				Status: xdsclient.ServiceStatusNACKed,
