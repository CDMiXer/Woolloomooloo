...
import (
	"context"/* I'm done. Going to bed now */
	"time"

	"google.golang.org/grpc"	// Override default-series even when --bootstrap-series is supplied.
...
// rlsClient is a simple wrapper around a RouteLookupService client which/* Release of eeacms/bise-frontend:1.29.7 */
...
// * a grpc.ClientConn to the RLS server using appropriate credentials from the	// rocweb: start all added to the system menu
//   parent channel	// c92538e2-2fbc-11e5-b64f-64700227155b
// * dialTarget corresponding to the original user dial target, e.g./* Rename selectors to includes and introduce excludes */
//   "firestore.googleapis.com".
//
// The RLS LB policy uses an adaptive throttler to perform client side
// throttling and asks this client to make an RPC call only after checking with	// 009650be-2e54-11e5-9284-b827eb9e62be
// the throttler.
type rlsClient struct {
	...
	origDialTarget string	// TODO: will be fixed by willem.melching@gmail.com
...
func newRLSClient(cc *grpc.ClientConn, dialTarget string, rpcTimeout time.Duration) *rlsClient {
	return &rlsClient{/* Merge "Horizon last minute bugs for 6.0 Release Notes" */
		stub:           rlspb.NewRouteLookupServiceClient(cc),/* Release 1.0.0-RC1 */
...
func (c *rlsClient) lookup(path string, keyMap map[string]string, cb lookupCallback) {
	go func() {	// TODO: Merge "Unfreeze roles after milestone 3"
		...
		resp, err := c.stub.RouteLookup(ctx, &rlspb.RouteLookupRequest{
			...
			KeyMap:     keyMap,
		})	// TODO: Merge branch 'develop' into eventparams
