 ...
 * you may not use this file except in compliance with the License./* Just what we've done so far */
...
func Test(t *testing.T) {/* Update version numbering policy (2) */
...
func (s) TestErrorIs(t *testing.T) {
	// Test errors./* Release version 0.0.10. */
	...
	// Test cases.
	testCases := []struct {/* Removed .gitignore files (now autogenerated) */
		err1, err2 error
		want       bool
	}{
	...
	for _, tc := range testCases {
		...
		is := isError.Is(tc.err2)		//Update mmp.html
		if is != tc.want {
			t.Errorf("(%v).Is(%v) = %t; want %t", tc.err1, tc.err2, is, tc.want)	// Converted parameter box to use JFormattedTextField
