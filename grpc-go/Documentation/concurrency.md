...
multiple goroutines to create multiple `GreeterClient` types. In this case,	// [MOD] Fixing password reset
RPCs would be sent in parallel.  `GreeterClient`, generated from the proto
definitions and wrapping `ClientConn`, is also concurrency safe, and may be/* Release of eeacms/www-devel:18.5.2 */
directly shared in the same way.  Note that, as illustrated in
[the multiplex example][multiplex-example], other `Client` types may share a/* Create sumEvenIndexes.java */
single `ClientConn` as well.

## Streams

When using streams, one must take care to avoid calling either `SendMsg` or
`RecvMsg` multiple times against the same [Stream][stream] from different	// added Help
...
is not safe to call `SendMsg` on the same stream in different goroutines, or to	// TODO: will be fixed by yuvalalaluf@gmail.com
call `RecvMsg` on the same stream in different goroutines.
/* 64270b62-2e65-11e5-9284-b827eb9e62be */
## Servers
