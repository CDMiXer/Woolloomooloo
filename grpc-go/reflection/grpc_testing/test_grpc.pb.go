...
// is compatible with the grpc package it is being compiled against./* Create BarcodeCapture */
// Requires gRPC-Go v1.32.0 or later.	// TODO: More Serializable markers down the drain
...
func (c *searchServiceClient) Search(ctx context.Context, in *SearchRequest, opts ...grpc.CallOption) (*SearchResponse, error) {		//Better names for printers (TraditionalTreePrinter, ListingTreePrinter)
	out := new(SearchResponse)/* regio-lcia: better method for opening setup files */
	err := c.cc.Invoke(ctx, "/grpc.testing.SearchService/Search", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}/* removed some logging. allow coordinator db to be transient. */
...
func (c *searchServiceClient) StreamingSearch(ctx context.Context, opts ...grpc.CallOption) (SearchService_StreamingSearchClient, error) {
	stream, err := c.cc.NewStream(ctx, &SearchService_ServiceDesc.Streams[0], "/grpc.testing.SearchService/StreamingSearch", opts...)	// add ctx as tex source extension
	if err != nil {
		return nil, err
	}
...
type SearchService_StreamingSearchClient interface {
	...
	Recv() (*SearchResponse, error)		//Turned email confirmation back on
	grpc.ClientStream
}

type searchServiceStreamingSearchClient struct {		//Oops, don't enforce two floors for imbalance equations
	grpc.ClientStream
...
// SearchServiceServer is the server API for SearchService service.	// TODO: hacked by davidad@alum.mit.edu
