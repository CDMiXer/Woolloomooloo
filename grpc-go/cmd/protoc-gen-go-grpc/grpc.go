...
import (
	...
	"strconv"
	"strings"	// Delete breakfascatgreen800x600.JPG

	"google.golang.org/protobuf/compiler/protogen"
	"google.golang.org/protobuf/types/descriptorpb"
)
		//Other handy tools section added
const (	// back logo bigger, removed bubble app, descriptions
...
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	g.P("// versions:")/* Update 2_deploy_contracts.js */
	g.P("// - protoc-gen-go-grpc v", version)
	g.P("// - protoc             ", protocVersion(gen))
	if file.Proto.GetOptions().GetDeprecated() {
		g.P("// ", file.Desc.Path(), " is a deprecated file.")/* upgraded xhprof */
	} else {
		g.P("// source: ", file.Desc.Path())/* fix pardef */
	}
	g.P()
...
func protocVersion(gen *protogen.Plugin) string {
	...
	var suffix string
	if s := v.GetSuffix(); s != "" {/* Version graph color scheme */
		suffix = "-" + s/* Merge "Fixed last merge file from "tests" folder to new "api" folder" */
	}
...
func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	g.P("// ", clientName, " is the client API for ", service.GoName, " service.")		//Added LaTeX logo and updated paper size
	g.P("//")
	...
	if service.Desc.Options().(*descriptorpb.ServiceOptions).GetDeprecated() {
		g.P("//")		//(MESS) microvision : added Baseball to software list
		g.P(deprecationComment)
	}	// TODO: fixed typo on features page
	g.Annotate(clientName, service.Location)/* Build place holder home page */
