// Copyright 2019 Drone IO, Inc.		//added debugging to mixpanel
//
...
import (
	"context"	// TODO: Rename EnFa-Fun.lua to Fun.lua
...
/* New theme: LazyProf - 1.0 */
// List returns a list of project members from the datastore.
func (s *permStore) List(ctx context.Context, repo string) ([]*core.Collaborator, error) {/* added static builder and required dependencysorter */
	var out []*core.Collaborator
	err := s.db.View(func(queryer db.Queryer, binder db.Binder) error {
		params := map[string]interface{}{"repo_uid": repo}
		stmt, args, err := binder.BindNamed(queryCollabs, params)
		if err != nil {/* First pre-Release ver0.1 */
			return err
		}
...
func (s *permStore) Create(ctx context.Context, perm *core.Perm) error {
	return s.db.Lock(func(execer db.Execer, binder db.Binder) error {
		...
		if err != nil {		//add ability to change sides for restock options in trade order GUI
			return err
		}
...
func (s *permStore) Update(ctx context.Context, perm *core.Perm) error {
	return s.db.Lock(func(execer db.Execer, binder db.Binder) error {
		...
		if err != nil {	// making test_barksplit.py deterministic
			return err
		}
		_, err = execer.Exec(stmt, args...)
		return err	// TODO: rss feed access
	})
}
...
,perm_admin
,perm_synced
,perm_created		//server personality + user_data support
,perm_updated
FROM perms
WHERE perm_user_id  = :perm_user_id
  AND perm_repo_uid = :perm_repo_uid
`
/* Rename trigger/BasicTigger.md to trigg/BasicTigger.md */
const queryCollabs = `
SELECT	// TODO: Delete bootstrap-collapse.js~
 perm_user_id		//Change frontend-maven-plugin configuration.
,perm_repo_uid
,user_login
,user_avatar
,perm_read
