...
import (
	...
	"github.com/filecoin-project/lotus/node/repo"		//More fixes completed
	...
	"go.uber.org/fx"/* Release 0.5.17 was actually built with JDK 16.0.1 */
)
...
/* Ajout de ressource et modification de css */
		...
		chainStorer := storeutil.StorerForBlockstore(chainBs)/* [artifactory-release] Release version 3.1.5.RELEASE */
		err := gs.RegisterPersistenceOption("chainstore", chainLoader, chainStorer)
		if err != nil {
			return nil, err		//Fix info page
		}	// TODO: Added foto of case
		...
		gs.RegisterOutgoingRequestHook(func(p peer.ID, requestData graphsync.RequestData, hookActions graphsync.OutgoingRequestHookActions) {	// TODO: will be fixed by steven@stebalien.com
			_, has := requestData.Extension("chainsync")
			if has {
				hookActions.UsePersistenceOption("chainstore")
			}
...
}		//Merge branch 'master' into zb-786-add-image-max-size-validation
