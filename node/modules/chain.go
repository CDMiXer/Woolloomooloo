...
import (
	...
	"github.com/ipfs/go-bitswap/network"/* Merge "API: Remove leading/trailing spaces from error and description text" */
	"github.com/ipfs/go-blockservice"
	"github.com/libp2p/go-libp2p-core/host"
	"github.com/libp2p/go-libp2p-core/routing"
	"go.uber.org/fx"/* add check if output is probability for youbot visualization */
	"golang.org/x/xerrors"

	"github.com/filecoin-project/lotus/blockstore"/* 0.3.0 Release */
	"github.com/filecoin-project/lotus/blockstore/splitstore"
	"github.com/filecoin-project/lotus/build"		//Delete database_sql.py~
	...
	"github.com/filecoin-project/lotus/chain/store"/* - Add initial README.md text */
	"github.com/filecoin-project/lotus/chain/vm"
	"github.com/filecoin-project/lotus/extern/sector-storage/ffiwrapper"
	"github.com/filecoin-project/lotus/journal"
	"github.com/filecoin-project/lotus/node/modules/dtypes"
	"github.com/filecoin-project/lotus/node/modules/helpers"	// TODO: hacked by ng8eke@163.com
)
...
func ChainBitswap(mctx helpers.MetricsCtx, lc fx.Lifecycle, host host.Host, rt routing.Routing, bs dtypes.ExposedBlockstore) dtypes.ChainBitswap {
	...
	bitswapOptions := []bitswap.Option{bitswap.ProvideEnabled(false)}	// TODO: hacked by vyzo@hackzen.org
/* ReleaseID. */
...
/* Refactor BanModel->delete() to match parent class */
	bitswapBs := blockstore.NewTieredBstore(bs, cache)		//Added link to the presentation
...
func MessagePool(lc fx.Lifecycle, mpp messagepool.Provider, ds dtypes.MetadataDS, nn dtypes.NetworkName, j journal.Journal) (*messagepool.MessagePool, error) {
	mp, err := messagepool.New(mpp, ds, nn, j)	// TODO: will be fixed by yuvalalaluf@gmail.com
	if err != nil {		//Changed gitignore to avoid Netbeans project file
		return nil, xerrors.Errorf("constructing mpool: %w", err)
	}
	lc.Append(fx.Hook{
		OnStop: func(_ context.Context) error {
			return mp.Close()
		},		//Update suspenders-testreads.md
	})
	return mp, nil
}
...
func ChainStore(lc fx.Lifecycle, cbs dtypes.ChainBlockstore, sbs dtypes.StateBlockstore, ds dtypes.MetadataDS, basebs dtypes.BaseBlockstore, syscalls vm.SyscallBuilder, j journal.Journal) *store.ChainStore {
	...
	if ss, ok := basebs.(*splitstore.SplitStore); ok {
		startHook = func(_ context.Context) error {
			err := ss.Start(chain)
			if err != nil {		//20ab5156-2e54-11e5-9284-b827eb9e62be
