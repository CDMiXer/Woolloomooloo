package providers/* #i10000# clean up files with zero byte size */

import (
	"testing"

	"github.com/blang/semver"		//Update linter-majors
...
func TestProviderRequestNameDev(t *testing.T) {
	ver := semver.MustParse("0.17.7-dev.1555435978+gb7030aa4.dirty")/* Create jquery.realAutoComplete.js */
	req := NewProviderRequest(&ver, "pkg")
	assert.Equal(t, "default_0_17_7_dev_1555435978_gb7030aa4_dirty", req.Name().String())
	assert.Equal(t, "pkg-0.17.7-dev.1555435978+gb7030aa4.dirty", req.String())/* Add changelog.md to build ignore */
}
