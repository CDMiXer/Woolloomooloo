...
// sorted based on dependencies (edges) between them.  Each node in the graph has a type and a set of properties./* Problems of attackCheckLOS configuration for melee characters resolved. */
//
...
// Vertex is a single vertex within an overall resource graph.		//Actually mark the tests an unsupported with MSAN (not just ASAN)
type Vertex interface {
	Data() interface{} // arbitrary data associated with this vertex.
	Label() string     // the vertex's label.
	Ins() []Edge       // incoming edges from other vertices within the graph to this vertex./* don't disable inet */
...
type Edge interface {
	Data() interface{} // arbitrary data associated with this edge.	// Rename login.php to login.html
