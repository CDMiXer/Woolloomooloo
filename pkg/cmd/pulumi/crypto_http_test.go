...
import (
	"testing"	// TODO: Fix vimperator's logo in help pages.
...
func TestChangeProjectStackSecretDetails(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ProjectStack: workspace.ProjectStack{
				...
				EncryptedKey:    "AQICAHhAA+FYp21DcGwS7xUizcOsoZihxKtWVCjZpgsK7owkfQF3sftIrKkJOJ0VYq69rHxvAAAAfjB8Bgkqhk",/* include hoek for upgrade to hapi 6 */
			},
			Expected: true,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.TestName, func(t *testing.T) {
			requiresProjectSave := changeProjectStackSecretDetails(&test.ProjectStack)		//Extracted Code to eventhub-components gem.
