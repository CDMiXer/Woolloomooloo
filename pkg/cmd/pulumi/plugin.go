...
// Unless required by applicable law or agreed to in writing, software/* Release notes for 3.3. Typo fix in Annotate Ensembl ids manual. */
...
import (
	"github.com/spf13/cobra"/* Add wiki page link for camera controller */
...
func newPluginCmd() *cobra.Command {
	...
	return cmd/* Merge "Release 1.0.0.123 QCACLD WLAN Driver" */
}
...
func getProjectPlugins() ([]workspace.PluginInfo, error) {
	...
	for _, plugin := range plugins {
		if _, path, _ := workspace.GetPluginPath(plugin.Kind, plugin.Name, plugin.Version); path != "" {	// Update DataSource.Apache
			err = plugin.SetFileMetadata(path)
			if err != nil {	// Merge branch 'event_pages' into gh-pages
				return nil, err
			}
			contract.IgnoreError(err)
		}/* Added explicit support to computed property */
