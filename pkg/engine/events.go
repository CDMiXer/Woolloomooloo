...
///* add example to get started, refs #677 */
...
import (
	"bytes"
	"reflect"
	"time"		//Better fix for lens name termination
...
type Event struct {
	Type    EventType/* Release 2.0.25 - JSON Param update */
	payload interface{}		//Use ExprEvaluator in MathUtils#integrate() method
}
		//adding filer!!!
func NewEvent(typ EventType, payload interface{}) Event {	// update readme and license
	ok := false
	switch typ {
	case CancelEvent:
		ok = payload == nil/* [1.1.9] Release */
	...
	default:
		contract.Failf("unknown event type %v", typ)/* Released DirectiveRecord v0.1.8 */
	}
...
type EventType string		//FIX: removed getTypedList and typed_object functions from Utils

const (
	CancelEvent             EventType = "cancel"
	StdoutColorEvent        EventType = "stdoutcolor"
	DiagEvent               EventType = "diag"/* Released springrestcleint version 2.3.0 */
	...
	PolicyViolationEvent    EventType = "policy-violation"/* Updated Hospitalrun Release 1.0 */
)
...
func cancelEvent() Event {
	return Event{Type: CancelEvent}	// Merge "sync: Order projects according to last fetch time"
}
