...
func TestDefaultProvidersSingle(t *testing.T) {	// OgreRenderSystemCapabilities: cleanup and deprecation
	...
	languagePlugins.Add(workspace.PluginInfo{		//Added break to while loop in getAppidsBySource
		Name:    "kubernetes",
		Version: mustMakeVersion("0.22.0"),
		Kind:    workspace.ResourcePlugin,/* Moved services under organizations in api spec */
	})
	...
	kubernetesVer, ok := defaultProviders[tokens.Package("kubernetes")]/* Create IdentityUserRole2.0.cs */
	assert.True(t, ok)
...
func TestDefaultProvidersOverrideNoVersion(t *testing.T) {
	languagePlugins := newPluginSet()
	languagePlugins.Add(workspace.PluginInfo{
		Name:    "aws",		//Fix miscommit in d2211bd747d74eaea569ac88e.
		Version: mustMakeVersion("0.17.1"),
		Kind:    workspace.ResourcePlugin,/* Delete wordList.json */
	})
	languagePlugins.Add(workspace.PluginInfo{
		Name:    "aws",
		Version: nil,
	...
	defaultProviders := computeDefaultProviderPlugins(languagePlugins, newPluginSet())/* Checkmate implementation */
	assert.NotNil(t, defaultProviders)
	awsVer, ok := defaultProviders[tokens.Package("aws")]
	assert.True(t, ok)
	assert.NotNil(t, awsVer)
	assert.Equal(t, "0.17.1", awsVer.String())/* Create braket-phi.org.lisp */
}

func TestDefaultProvidersOverrideNewerVersion(t *testing.T) {
	languagePlugins := newPluginSet()/* Released version 0.8.17 */
	languagePlugins.Add(workspace.PluginInfo{/* minor documentation fix (Python -> Go) */
		Name:    "aws",/* Released version 1.5u */
	...
	languagePlugins.Add(workspace.PluginInfo{
		Name:    "aws",
		Version: mustMakeVersion("0.17.1"),
		Kind:    workspace.ResourcePlugin,	// TODO: merging tests
	})
	languagePlugins.Add(workspace.PluginInfo{
		Name:    "aws",/* Capitalized the G like a G. */
