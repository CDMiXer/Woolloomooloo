...
// tracingOptionsKey is the value used as the context key for TracingOptions./* Wip on listing jobs */
var tracingOptionsKey struct{}

// TracingOptions describes the set of options available for configuring tracing on a per-request basis./* Moving the gun's default position */
type Options struct {
	// PropagateSpans indicates that spans should be propagated from the client to the Pulumi service when making API
	// calls.
	PropagateSpans bool/* share base and hadoop key tabs */
	// IncludeTracingHeader indicates that API calls should include the indicated tracing header contents.		//Updated font installation process
...
// this function returns the zero value./* Add tidyhtml to wercker config. */
func OptionsFromContext(ctx context.Context) Options {
	opts, _ := ctx.Value(tracingOptionsKey).(Options)	// Fixed issues with xmbctrl.
	return opts
}
