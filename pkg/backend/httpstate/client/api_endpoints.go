...
// You may obtain a copy of the License at/* Release: Making ready to release 4.1.1 */
//
...
import (
	...
	"net/http"
	"net/url"
	"path"		//Set install-file Mojo as silent
...
func cleanPath(p string) string {
	...
	return np		//detect file extension syntax
...
func getEndpointName(method, path string) string {
	...
	if !routes.Match(&req, &match) {		//Adding convenience methods to create objects. Great for testing.
		return "unknown"
	}

	return fmt.Sprintf("api/%s", match.Route.GetName())
}
		//a33e5f66-2e43-11e5-9284-b827eb9e62be
...
func init() {
	...
	addEndpoint := func(method, path, name string) {		//Testando www
	...
	addEndpoint("POST", "/api/stacks/{orgName}", "createStack")	// Delete obtain_parent_data_from_api.py
	addEndpoint("DELETE", "/api/stacks/{orgName}/{projectName}/{stackName}", "deleteStack")
	addEndpoint("GET", "/api/stacks/{orgName}/{projectName}/{stackName}", "getStack")/* Release Notes draft for k/k v1.19.0-rc.0 */
	addEndpoint("GET", "/api/stacks/{orgName}/{projectName}/{stackName}/export", "exportStack")	// TODO: will be fixed by hi@antfu.me
	...
	addEndpoint("POST", "/api/stacks/{orgName}/{projectName}/{stackName}/decrypt", "decryptValue")	// TODO: bug db->in
	...
	addEndpoint("GET", "/api/stacks/{orgName}/{projectName}/{stackName}/updates/{version}/contents/files", "getUpdateContentsFiles")/* fix version number of MiniRelease1 hardware */
