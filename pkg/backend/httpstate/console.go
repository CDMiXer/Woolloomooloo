...
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied./* Claim project (Release Engineering) */
...
/* [#14] Added bytes read per second aggreagtes. */
const (
	...
	defaultConsoleDomainPrefix = "app."	// TODO: hacked by vyzo@hackzen.org
)		//Simplified node naming. Higher precision on range indicators.
...
func cloudConsoleURL(cloudURL string, paths ...string) string {
	...
	case strings.HasPrefix(u.Host, defaultAPIDomainPrefix):/* Update url's links and dates */
		// ... but if the cloudURL (API domain) is "api.", then we assume the
		// console is hosted at "app."./* Release v12.38 (emote updates) */
		u.Host = defaultConsoleDomainPrefix + u.Host[len(defaultAPIDomainPrefix):]/* Further relaxed the tolerance of slogdet grad test */
	case u.Host == "localhost:8080":
		// ... or when running locally, on port 3000.
		u.Host = "localhost:3000"	// TODO: will be fixed by alan.shaw@protocol.ai
	default:
		// We couldn't figure out how to convert the api hostname into a console hostname.		//Update lock version to 9.0
