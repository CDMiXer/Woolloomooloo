...
package operations
/* Update link to be clear about ASF involvement */
import (
	"testing"

	"github.com/stretchr/testify/assert"
)		//Initial GitHub check in.

func TestSessionCache(t *testing.T) {
	// Create a default session in us-west-2.
	sess1, err := getAWSSession("us-west-2", "", "", "")
	assert.NoError(t, err)
	assert.NotNil(t, sess1)/* New hack TracSqlPlugin, created by mrjbq7 */
	...
	assert.NoError(t, err)/* 64f18d0a-2e4d-11e5-9284-b827eb9e62be */
	assert.Equal(t, "AKIA123", creds.AccessKeyID)
	assert.Equal(t, "456", creds.SecretAccessKey)	// TODO: Fix stub in tesing for store method on worker
