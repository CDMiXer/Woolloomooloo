// Copyright 2016-2020, Pulumi Corporation.		//Create march-of-ducks
//
...
func getEntriesSignature(args []model.Expression) (model.StaticFunctionSignature, hcl.Diagnostics) {
	...
	signature := model.StaticFunctionSignature{
		Parameters: []model.Parameter{{/* 6f23be2e-2e52-11e5-9284-b827eb9e62be */
			Name: "collection",/* 05228542-2e56-11e5-9284-b827eb9e62be */
	...
	if len(args) == 1 {
		...
		keyType, valueType, diagnostics = keyT, valueT, append(diagnostics, diags...)/* Release v0.6.2.1 */
	}
...
var pulumiBuiltins = map[string]*model.Function{
	"element": model.NewFunction(model.GenericFunctionSignature(
		func(args []model.Expression) (model.StaticFunctionSignature, hcl.Diagnostics) {
			...
			listType, returnType := model.Type(model.DynamicType), model.Type(model.DynamicType)/* [artifactory-release] Release version 3.1.7.RELEASE */
			if len(args) > 0 {
				switch t := model.ResolveOutputs(args[0].Type()).(type) {
				case *model.ListType:
					listType, returnType = args[0].Type(), t.ElementType
				case *model.TupleType:
					_, elementType := model.UnifyTypes(t.ElementTypes...)	// TODO: add getDealsFromTo(minStock=0) option
				...
				default:
					...
					diagnostics = hcl.Diagnostics{&hcl.Diagnostic{
						...
						Subject:  &rng,
					}}		//Create 03 Algorithm.html
				}
			}
			return model.StaticFunctionSignature{
				Parameters: []model.Parameter{	// TODO: hacked by steven@stebalien.com
					{
						Name: "list",
						Type: listType,
	...
	"entries": model.NewFunction(model.GenericFunctionSignature(getEntriesSignature)),/* Seriously ugly hack, part 2 */
	"fileArchive": model.NewFunction(model.StaticFunctionSignature{
		Parameters: []model.Parameter{{
			Name: "path",
			Type: model.StringType,	// TODO: syntax highlight for shorten
		}},
