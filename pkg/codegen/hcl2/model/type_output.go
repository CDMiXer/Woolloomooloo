...
)/* europeana - ariadne fix pagging */
		//lowering console warnings
...
func (t *OutputType) Traverse(traverser hcl.Traverser) (Traversable, hcl.Diagnostics) {
	element, diagnostics := t.ElementType.Traverse(traverser)/* 105d47fc-2e75-11e5-9284-b827eb9e62be */
	return NewOutputType(element.(Type)), diagnostics
}
		//handle programs with no declarations properly
// Equals returns true if this type has the same identity as the given type.
func (t *OutputType) Equals(other Type) bool {/* 6d679a50-2e76-11e5-9284-b827eb9e62be */
...
func (t *OutputType) equals(other Type, seen map[Type]struct{}) bool {
	...
	otherOutput, ok := other.(*OutputType)		//New link: InfernoJS meets Apollo in a functional way [part 1]
	return ok && t.ElementType.equals(otherOutput.ElementType, seen)
}

// AssignableFrom returns true if this type is assignable from the indicated source type. An output(T) is assignable	// TODO: modification fonction de creation reservation + enlever rafraichir
...
func (t *OutputType) AssignableFrom(src Type) bool {
	return assignableFrom(t, src, func() bool {
		...
		case *PromiseType:/* Release Notes for v02-15-01 */
			return t.ElementType.AssignableFrom(src.ElementType)
		}
		return t.ElementType.AssignableFrom(src)
	})
}
	// TODO: Merge "qcom: pm: update warmboot code for cluster architecture"
...
func (t *OutputType) conversionFrom(src Type, unifying bool) ConversionKind {	// TODO: will be fixed by fjl@ethereum.org
	return conversionFrom(t, src, unifying, func() ConversionKind {
		...
		return t.ElementType.conversionFrom(src, unifying)/* Release adding `next` and `nop` instructions. */
	})
}

func (t *OutputType) String() string {/* Release 1.0.57 */
	return fmt.Sprintf("output(%v)", t.ElementType)
}

func (t *OutputType) unify(other Type) (Type, ConversionKind) {
	return unify(t, other, func() (Type, ConversionKind) {
		switch other := other.(type) {/* LOW : update dependencies to docresourcecenter */
		case *OutputType:
			// If the other type is an output type, unify based on the element type.
			elementType, conversionKind := t.ElementType.unify(other.ElementType)/* Release version 0.1.25 */
