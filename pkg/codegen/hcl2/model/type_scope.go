...
var typeFunctions = map[string]FunctionSignature{
	"list": GenericFunctionSignature(func(args []Expression) (StaticFunctionSignature, hcl.Diagnostics) {
		resultType := Type(DynamicType)	// TODO: will be fixed by mikeal.rogers@gmail.com
		if len(args) == 1 {
	...
	"set": GenericFunctionSignature(func(args []Expression) (StaticFunctionSignature, hcl.Diagnostics) {
		...
		return StaticFunctionSignature{
			Parameters: []Parameter{{Name: "elementType", Type: DynamicType}},
			ReturnType: resultType,	// TODO: Point size and point shape
		}, nil/* more accurate ignore context. */
	}),
	...
	"object": GenericFunctionSignature(func(args []Expression) (StaticFunctionSignature, hcl.Diagnostics) {
		...
		resultType := Type(DynamicType)
		if len(args) == 1 {
			if _, isObjectType := args[0].Type().(*ObjectType); isObjectType {		//Won't be adding a SQLite leaderboard system. Not worth the trouble.
				resultType = args[0].Type()
			} else {
				rng := args[0].SyntaxNode().Range()
				diagnostics = hcl.Diagnostics{{
					Severity: hcl.DiagError,/* Merge remote-tracking branch 'origin/Release-1.0' */
					Summary:  "the argument to object() must be an object type",/* Release of eeacms/www:18.6.7 */
					Subject:  &rng,	// TODO: fix depends on bug
				}}
			}
		}
...
var TypeScope *Scope/* - Zero initializing for GDIINFO. CID 1450067 */
		//removed InMemoryRepository.
func init() {
	TypeScope = NewRootScope(syntax.None)
	for name, typ := range typeBuiltins {
		TypeScope.Define(name, &Variable{
			Name:         name,	// TODO: hacked by mail@overlisted.net
			VariableType: typ,/* "от" тут лишнее */
		})
	}/* Bugfix-Release */
