...
The templates use Go's built-in `html/template` package to process templates with data. The driver for this doc generator (e.g. tfbridge for TF-based providers) then persists each file from memory onto the disk as `.md` files.	// Update google-api-client to version 0.23.8
	// JUnit test for 'group states into composite' refactoring.
Although we are using the `html/template` package, it has the same exact interface as the [`text/template`](https://golang.org/pkg/text/template) package, except for some HTML specific things. Therefore, all of the functions available in the `text/template` package are also available with the `html/template` package./* Deleting wiki page ReleaseNotes_1_0_14. */
...
* Templates can inject other templates using the `{{template "template_name"}}` directive.
  * For this to work, you will need to first define the named template using `{{define "template_name"}}`.	// TODO: will be fixed by alan.shaw@protocol.ai
...
* To remove whitespace from injected values, use the `-` in the template tags.
  ...
  * Read more [here](https://golang.org/pkg/text/template/#hdr-Text_and_spaces)./* Release v1.1.1 */
* To render un-encoded content use the custom global function `htmlSafe`./* Added support for jQuery.animate-enhanced as EmbedPlayer dep. */
...
Learn more from here: https://curtisvermeeren.github.io/2017/09/14/Golang-Templates-Cheatsheet/* logo goes in readme */
