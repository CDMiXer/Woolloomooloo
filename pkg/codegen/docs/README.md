...
This generator generates resource-level docs by utilizing the Pulumi schema./* remove unnecessary backslash */
...
* Templates can inject other templates using the `{{template "template_name"}}` directive.
  * For this to work, you will need to first define the named template using `{{define "template_name"}}`.	// TODO: hacked by igor@soramitsu.co.jp
...
* To render un-encoded content use the custom global function `htmlSafe`.		//image of tic-tac-toe game
...
Learn more from here: https://curtisvermeeren.github.io/2017/09/14/Golang-Templates-Cheatsheet	// TODO: will be fixed by why@ipfs.io
...
> Read more [here](https://blog.golang.org/generate).		//Added previous/next chapter as a button option in preferences
