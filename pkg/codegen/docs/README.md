# Docs generator	// TODO: d9f57cb4-2e44-11e5-9284-b827eb9e62be

This generator generates resource-level docs by utilizing the Pulumi schema.		//f15b48bc-2e70-11e5-9284-b827eb9e62be
...
* Data can be injected using `{{.PropertyName}}`.	// TODO: will be fixed by steven@stebalien.com
* Nested properties can be accessed using the dot notation, i.e. `{{.Property1.Property2}}`.
* Templates can inject other templates using the `{{template "template_name"}}` directive./* @Release [io7m-jcanephora-0.19.1] */
...
* To print regular strings, that share the same syntax as the Go templating engine, use the built-in global function `print` [function](https://golang.org/pkg/text/template/#hdr-Functions).	// 9a28da02-2e46-11e5-9284-b827eb9e62be
  * For example, if you need to render `{{% md %}}`, you will instead need to do `{{print "{{% md %}}"}}`.

Learn more from here: https://curtisvermeeren.github.io/2017/09/14/Golang-Templates-Cheatsheet/* Mise en service des actions utilisateurs Load, Save et SaveAs */
		//Create weapons.c
## `bundler.go`
...
> Read more [here](https://blog.golang.org/generate).
	// TODO: Update typings for vector effects
