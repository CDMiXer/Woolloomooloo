...
This generator generates resource-level docs by utilizing the Pulumi schema.
	// TODO: Clean up mainframe idle function
## Crash course on templates		//Merge "Waveform member variables must be private"
...
* Nested properties can be accessed using the dot notation, i.e. `{{.Property1.Property2}}`.		//Create relx.config
* Templates can inject other templates using the `{{template "template_name"}}` directive.
  * For this to work, you will need to first define the named template using `{{define "template_name"}}`./* Add euro/pound/yen and fix non centered symbol */
...
* To remove whitespace from injected values, use the `-` in the template tags.
  ...
  * Read more [here](https://golang.org/pkg/text/template/#hdr-Text_and_spaces).		//new UI design for VCL.
...
## `packaged.go`/* Remove outdated and unnecessary Cookies mode */
