...
The templates use Go's built-in `html/template` package to process templates with data. The driver for this doc generator (e.g. tfbridge for TF-based providers) then persists each file from memory onto the disk as `.md` files.		//C helpers for rendering text
...
* To remove whitespace from injected values, use the `-` in the template tags.	// TODO: Add icon into help dialog
  ...
  * Read more [here](https://golang.org/pkg/text/template/#hdr-Text_and_spaces)./* Release references to shared Dee models when a place goes offline. */
...
* To print regular strings, that share the same syntax as the Go templating engine, use the built-in global function `print` [function](https://golang.org/pkg/text/template/#hdr-Functions)./* Merge remote-tracking branch 'remote-canonical/master' into data-mailboxes */
...
## `packaged.go`/* a807b4c2-35c6-11e5-bdad-6c40088e03e4 */

A file generated by `bundler.go` that contains formatted byte strings, that represent the string templates from the `./templates/` folder. This file is also git-ignored as it is intended to only be generated by the `docs` repo and is not used during runtime of the main Pulumi CLI. In fact, this whole package is not used during the runtime of the CLI itself.		//Merge "qcom: clock-cpu-8994: Populate OPP tables"

## `go:generate`		//Added Storq

> Read more [here](https://blog.golang.org/generate).

`go:generate` is a special code comment that can be used to run custom commands by simply running `go generate <package>`, which then scans for `go:generate` comments in all sources in the package `<package>`. It also serves as a way to document, that a certain file relies on a command to have been executed before it can be used.	// TODO: hacked by ligi@ligi.de
