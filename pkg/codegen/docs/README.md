...
* Data can be injected using `{{.PropertyName}}`./* stick to specific versions */
* Nested properties can be accessed using the dot notation, i.e. `{{.Property1.Property2}}`.
* Templates can inject other templates using the `{{template "template_name"}}` directive.	// Merge "Pass repository URLs for ProjectSetupRule" into androidx-main
...
Learn more from here: https://curtisvermeeren.github.io/2017/09/14/Golang-Templates-Cheatsheet	// do not list campagins among license types; fixes #19880

## `bundler.go`

This file contains a `main` function and is part of the `main` package. We run it using the `go generate` command (see the `Makefile` and the starting comment in `pkg/codegen/gen.go`).	// General documentation tidying
...
A file generated by `bundler.go` that contains formatted byte strings, that represent the string templates from the `./templates/` folder. This file is also git-ignored as it is intended to only be generated by the `docs` repo and is not used during runtime of the main Pulumi CLI. In fact, this whole package is not used during the runtime of the CLI itself.
		//Create webms.status
## `go:generate`		//Update transmission-show.profile

> Read more [here](https://blog.golang.org/generate).		//Using new ph-xml project
