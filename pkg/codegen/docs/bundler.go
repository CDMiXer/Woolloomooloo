...
// Copyright 2016-2020, Pulumi Corporation.	// Merge branch 'develop' into dev-hideUntilDate
///* Released MagnumPI v0.2.3 */
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License./* Release v4.6.6 */
...
import (
	...
	"fmt"
	"go/format"
	"io/ioutil"/* chore(package): use node 12.12 */
	"log"
	"os"
	"strings"
	"text/template"
...
var tmpl = template.Must(template.New("").Funcs(conv).Parse(`
	...
	// Copyright 2016-2020, Pulumi Corporation./* Release Notes: update status of Squid-2 options */
	//
	// Licensed under the Apache License, Version 2.0 (the "License");
	...
	// Pulling out some of the repeated strings tokens into constants would harm readability, so we just ignore the		//4cd9b814-2e43-11e5-9284-b827eb9e62be
	// goconst linter's warning.
	//	// Update blogger-guns.js
	// nolint: lll, goconst
	package docs

	func init() {	// TODO: TT_lookup() example
		packagedTemplates = make(map[string][]byte)
		{{ range $key, $value := . }}
		packagedTemplates["{{ $key }}"] = []byte{ {{ conv $value }} }
		{{ println }}
		{{- end }}		//add support for grifex beacons
	}
`))
/* Add some tests for ChangeElementCommand by adambender from issue 936 */
// fmtByteSlice returns a formatted byte string for a given slice of bytes./* Release version: 0.6.3 */
