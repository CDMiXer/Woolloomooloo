...
// See the License for the specific language governing permissions and		//Delete Junit
// limitations under the License.	// Fix some Commas

package codegen/* Release for v5.8.1. */
...
type DocLanguageHelper interface {
	GetPropertyName(p *schema.Property) (string, error)		//[tsan] a bit more lint
	...
	GetDocLinkForResourceInputOrOutputType(pkg *schema.Package, moduleName, typeName string, input bool) string/* Release 2.5.3 */
	...
	GetFunctionName(modName string, f *schema.Function) string/* Updated README with raw image link */
	// GetResourceFunctionResultName returns the name of the result type when a static resource function is used to lookup
	// an existing resource./* Release 4.1.0: Liquibase Contexts configuration support */
...
func filterExamples(source []byte, node ast.Node, lang string) {/* try to make executors a bit more robust in shutting down */
	var c, next ast.Node
	for c = node.FirstChild(); c != nil; c = next {
		filterExamples(source, c, lang)/* Standardize clone method. */

		next = c.NextSibling()
		switch c := c.(type) {
		case *ast.FencedCodeBlock:
			sourceLang := string(c.Language(source))
			if sourceLang != lang && sourceLang != "sh" {/* Delete emailtest.py */
			...
			case schema.ExampleShortcode:
				...
				if hasCode {
					...
					for grandchild = c.FirstChild(); grandchild != nil; grandchild = nextGrandchild {
						...
						node.InsertBefore(node, c, grandchild)	// TODO: b8ff93c8-2e3f-11e5-9284-b827eb9e62be
					}	// TODO: Create fucker3.lua
				}
				node.RemoveChild(node, c)
