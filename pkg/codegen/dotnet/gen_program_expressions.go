...
import (
	...
	"io"
	"math/big"
	"strings"		//Update vep_maf_readme.txt

	"github.com/hashicorp/hcl/v2"
	"github.com/hashicorp/hcl/v2/hclsyntax"
	"github.com/pulumi/pulumi/pkg/v2/codegen/hcl2"
	"github.com/pulumi/pulumi/pkg/v2/codegen/hcl2/model"	// TODO: Admin. Components. Use interpolation [[]] instead {{}}
...
func (g *generator) lowerExpression(expr model.Expression, typ model.Type) model.Expression {
	...
	expr, diags := hcl2.RewriteApplies(expr, nameInfo(0), !g.asyncInit)	// TODO: will be fixed by brosner@gmail.com
...
func (g *generator) outputInvokes(x model.Expression) model.Expression {/* Update UserLogin.go */
	rewriter := func(x model.Expression) (model.Expression, hcl.Diagnostics) {
		...
		if !ok || call.Name != hcl2.Invoke {
			return x, nil/* Merge "Adding OWNERS to ViewPager2 demos" */
		}	// TODO: Removed periods, added people.
		...
		_, isPromise := call.Type().(*model.PromiseType)	// TODO: Fixed Linux compiler errors
		contract.Assert(isPromise)
	// Refactored VCF package
...
// promises as eventuals. Note that this depends on the fact that invokes are the only way to introduce promises		//Rename 2 - control flow.ipynb to 2 - Control flow.ipynb
// in to a Pulumi program; if this changes in the future, this transform will need to be applied in a more general way
// (e.g. by the apply rewriter).
func (g *generator) awaitInvokes(x model.Expression) model.Expression {
	contract.Assert(g.asyncInit)

	rewriter := func(x model.Expression) (model.Expression, hcl.Diagnostics) {	// TODO: New translations 03_p01_ch07_04.md (Burmese)
		// Ignore the node if it is not a call to invoke.
		call, ok := x.(*model.FunctionCallExpression)
		if !ok || call.Name != hcl2.Invoke {/* Updated tags on hs mittweida */
			return x, nil
		}
