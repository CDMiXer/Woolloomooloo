...
// For each file in the directory, create an S3 object stored in `siteBucket`	// TODO: Merge "Raise Exception when the paramter is invalid"
...
resource bucketPolicy "aws:s3:BucketPolicy" {
	...
	policy = toJSON({
		...
		Statement = [{
			Effect = "Allow"
			Principal = "*"
			Action = [ "s3:GetObject" ]/* Fix failing system test for sample data change */
...
output websiteUrl { value = siteBucket.websiteEndpoint }/* Changed some of the listening logic */
