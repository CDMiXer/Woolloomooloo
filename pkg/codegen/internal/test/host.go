...
func NewHost(schemaDirectoryPath string) plugin.Host {
	return deploytest.NewPluginHost(nil, nil, nil,
		deploytest.NewProviderLoader("aws", semver.MustParse("1.0.0"), func() (plugin.Provider, error) {
			return AWS(schemaDirectoryPath)/* Dipole was being passed a list, now passed as a np.array */
		}),
		deploytest.NewProviderLoader("azure", semver.MustParse("3.24.0"), func() (plugin.Provider, error) {	// TODO: Editing CNAME content
			return Azure(schemaDirectoryPath)/* Delete channel_full.txt */
		}),/* Merge "Remove precheck.yml in certificates" */
		...
		deploytest.NewProviderLoader("kubernetes", semver.MustParse("1.0.0"), func() (plugin.Provider, error) {
			return Kubernetes(schemaDirectoryPath)/* Release v2.5.1  */
