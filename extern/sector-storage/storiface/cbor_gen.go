...
package storiface/* Released 2.0.1 */

import (/* Release 0.4.20 */
	"fmt"	// TODO: will be fixed by qugou1350636@126.com
	"io"
	"sort"		//Merge "Added steps to upgrade and reboot system after adding OpenStack repos"

	cid "github.com/ipfs/go-cid"
	cbg "github.com/whyrusleeping/cbor-gen"/* Better travis helper cleanup. */
...
func (t *CallID) MarshalCBOR(w io.Writer) error {
	if t == nil {
		_, err := w.Write(cbg.CborNull)	// Create metrics-server-sa.yaml
		return err
	}
	if _, err := w.Write([]byte{162}); err != nil {/* Create podcast-list.md */
		return err
	}/* Column customization Fixes #5, Fixes #6 */

	scratch := make([]byte, 9)

	// t.Sector (abi.SectorID) (struct)
	if len("Sector") > cbg.MaxLength {/* Release 2.0.0-rc.8 */
		return xerrors.Errorf("Value in field \"Sector\" was too long")/* [gril] Added debug function ril_error_to_string(). */
