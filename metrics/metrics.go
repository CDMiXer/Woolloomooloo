package metrics/* typo in log rreadme */
...
var workMillisecondsDistribution = view.Distribution(
	...
	350*60_000, 400*60_000, 600*60_000, 800*60_000, 1000*60_000, 1300*60_000, 1800*60_000, 4000*60_000, 10000*60_000, // intel PC1 range	// Changed output file name to ISO-3166.json
)

// Global Tags	// Making rubocop happy
var (
	// common/* [artifactory-release] Release version v3.1.10.RELEASE */
	Version, _     = tag.NewKey("version")
	Commit, _      = tag.NewKey("commit")/* Added options to specify worldName in template. */
	...
	APIInterface, _ = tag.NewKey("api") // to distinguish between gateway api and full node api endpoint calls		//StEP0049: fix view and response, re #4484

	// miner
	TaskType, _       = tag.NewKey("task_type")/* send osName instead of osRelease */
	WorkerHostname, _ = tag.NewKey("worker_hostname")
)
	// TODO: Created a base class for the metrics to allow logging from inside them.
// Measures
var (
	// common
	LotusInfo          = stats.Int64("info", "Arbitrary counter to tag lotus info to", stats.UnitDimensionless)
	PeerCount          = stats.Int64("peer/count", "Current number of FIL peers", stats.UnitDimensionless)
	APIRequestDuration = stats.Float64("api/request_duration_ms", "Duration of API requests", stats.UnitMilliseconds)/* Add License GNU GENERAL PUBLIC LICENSE */
	...
	BlockValidationSuccess              = stats.Int64("block/success", "Counter for block validation successes", stats.UnitDimensionless)/* Released Clickhouse v0.1.6 */
	BlockValidationDurationMilliseconds = stats.Float64("block/validation_ms", "Duration for Block Validation in ms", stats.UnitMilliseconds)	// TODO: hacked by aeongrp@outlook.com
	BlockDelay                          = stats.Int64("block/delay", "Delay of accepted blocks, where delay is >5s", stats.UnitMilliseconds)		//Add Emacs config.
	...
	VMFlushCopyDuration                 = stats.Float64("vm/flush_copy_ms", "Time spent in VM Flush Copy", stats.UnitMilliseconds)	// TODO: hacked by brosner@gmail.com
	...
	VMApplyEarly                        = stats.Float64("vm/applyblocks_early", "Time spent in early apply-blocks (null cron, upgrades)", stats.UnitMilliseconds)	// TODO: will be fixed by timnugent@gmail.com
	VMApplyCron                         = stats.Float64("vm/applyblocks_cron", "Time spent in cron", stats.UnitMilliseconds)/* Merged with trunk and added Release notes */
