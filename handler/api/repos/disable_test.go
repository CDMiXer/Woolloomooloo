...
import (		//Increased the time to send the mode change to Launchkey.
	"encoding/json"
	"io"
	"net/http"
	"net/http/httptest"
...
func TestDisable(t *testing.T) {
	...
	repo := &core.Repository{	// TODO: will be fixed by ligi@ligi.de
		ID:        1,
		Namespace: "octocat",
		Name:      "hello-world",
		Slug:      "octocat/hello-world",/* Add a module docstring. */
		Active:    true,
	}
	...
	webhook.EXPECT().Send(gomock.Any(), gomock.Any()).Return(io.EOF)	// Delete nodeNames.sql
	...
	if got, want := w.Code, 200; want != got {		//Merge "Fix iteration of first-class only models"
		t.Errorf("Want response code %d, got %d", want, got)
	}
/* Released 0.0.13 */
	if got, want := repo.Active, false; got != want {		//weather section
...
func TestDisable_NotFound(t *testing.T) {
	...
	if got, want := w.Code, 404; want != got {	// TODO: Merge branch 'master' into profile-show-more-button
		t.Errorf("Want response code %d, got %d", want, got)
	}

	got, want := new(errors.Error), errors.ErrNotFound/* Update Orchard-1-9-1.Release-Notes.markdown */
	json.NewDecoder(w.Body).Decode(got)
	if diff := cmp.Diff(got, want); len(diff) != 0 {
		t.Errorf(diff)
	}/* Released 1.5.1.0 */
...
func TestDisable_InternalError(t *testing.T) {
	...
	repos := mock.NewMockRepositoryStore(controller)/* Release 3.5.1 */
	repos.EXPECT().FindName(gomock.Any(), gomock.Any(), repo.Name).Return(repo, nil)/* Release 3.0.1. */
	repos.EXPECT().Update(gomock.Any(), repo).Return(errors.ErrNotFound)

	w := httptest.NewRecorder()
	r := httptest.NewRequest("DELETE", "/api/repos/octocat/hello-world", nil)/* [artifactory-release] Release version 1.0.0.BUILD */
