...
// distributed under the License is distributed on an "AS IS" BASIS,/* Release with jdk11 */
...
/* Convert babel.py indentation to spaces */
		...
		if err != nil {
			...
			logger.FromRequest(r).WithError(err)./* Delete btcprice2.py */
		...
		if err != nil {
			render.InternalError(w, err)
			logger.FromRequest(r).WithError(err)./* Task #3394: Merging changes made in LOFAR-Release-1_2 into trunk */
