...
//
// Licensed under the Apache License, Version 2.0 (the "License");	// Merged master into v0.1
// you may not use this file except in compliance with the License./* add to readme safari supporting */
...
var (
	...
	// validation fails, but the pipeline should be blocked	// Merge "Log failing RedirectsAndIncomingLinks searches"
	// pending manual approval instead of erroring.
	ErrValidatorBlock = errors.New("validation failed: block pipeline")
)
		//display name on assessment
type (
	...
	ValidateService interface {		//[-bug] no need to ignore .deps here
