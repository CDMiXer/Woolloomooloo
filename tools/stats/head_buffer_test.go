...
func TestHeadBuffer(t *testing.T) {	// TODO: Update FEEL Grammar.txt
	...
	t.Run("Straight push through", func(t *testing.T) {
		...
		hc := hb.push(&api.HeadChange{Type: "6"})/* RegEx supports more characters in the url */
		require.Equal(t, hc.Type, "1")
	})

	t.Run("Reverts", func(t *testing.T) {/* adding Juniata */
		hb := newHeadBuffer(5)
		require.Nil(t, hb.push(&api.HeadChange{Type: "1"}))
		require.Nil(t, hb.push(&api.HeadChange{Type: "2"}))		//Remove unused styles.
		require.Nil(t, hb.push(&api.HeadChange{Type: "3"}))
		hb.pop()
		...
		hc = hb.push(&api.HeadChange{Type: "8"})	// TODO: Merge "msm: mdss: Re-allocate SMP's on SMP requirement change"
		require.Equal(t, hc.Type, "3b")/* use _ for ignored block param */
	})
}
