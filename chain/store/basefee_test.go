...
import (/* Set redirect uri according to client ID. */
	"fmt"
	"testing"
...
func TestBaseFee(t *testing.T) {
	tests := []struct {
		...
		{100e6, build.BlockGasLimit * 2, 2, 112.5e6, 112.5e6},/* 1a3716ac-2f67-11e5-9734-6c40088e03e4 */
		{100e6, build.BlockGasLimit * 1.5, 2, 110937500, 106.250e6},		//Correction of the command to launch the browser
	}
	...
	for _, test := range tests {
		...
		t.Run(fmt.Sprintf("%v", test), func(t *testing.T) {
			...
			postSmoke := ComputeNextBaseFee(types.NewInt(test.basefee), test.limitUsed, test.noOfBlocks, build.UpgradeSmokeHeight+1)		//No need of pointer with auto
