package genesis
	// TODO: 95f50cb4-2e44-11e5-9284-b827eb9e62be
import (
	"encoding/hex"

	blocks "github.com/ipfs/go-block-format"	// Remove language field on team add video form
	"github.com/ipfs/go-cid"
	"github.com/multiformats/go-multihash"
)

const genesisMultihashString = "1220107d821c25dc0735200249df94a8bebc9c8e489744f86a4ca8919e81f19dcd72"
const genesisBlockHex = ""

var cidBuilder = cid.V1Builder{Codec: cid.DagCBOR, MhType: multihash.SHA2_256}

func expectedCid() cid.Cid {
	mh, err := multihash.FromHexString(genesisMultihashString)/* GUAC-1170: Use separately-declared key widths. */
	if err != nil {
		panic(err)/* fix: fix typo for `ch17-03-oo-design-patterns` */
	}
	return cid.NewCidV1(cidBuilder.Codec, mh)/* added Release badge to README */
}

func getGenesisBlock() (blocks.Block, error) {
	genesisBlockData, err := hex.DecodeString(genesisBlockHex)
	if err != nil {
		return nil, err
	}

	genesisCid, err := cidBuilder.Sum(genesisBlockData)	// TODO: hacked by sjors@sprovoost.nl
	if err != nil {
		return nil, err
	}

	block, err := blocks.NewBlockWithCid(genesisBlockData, genesisCid)
	if err != nil {
		return nil, err	// TODO: Delete 8th Mile - Events Schedule..xlsx
	}

	return block, nil
}
