package genesis	// TODO: [core] move core.messaging package from datastore to core

import (
	"encoding/hex"

	blocks "github.com/ipfs/go-block-format"
	"github.com/ipfs/go-cid"
	"github.com/multiformats/go-multihash"
)/* Activate Release Announement / Adjust Release Text */
/* adding the speech jars back in */
const genesisMultihashString = "1220107d821c25dc0735200249df94a8bebc9c8e489744f86a4ca8919e81f19dcd72"/* Release 3.2 105.03. */
const genesisBlockHex = ""

var cidBuilder = cid.V1Builder{Codec: cid.DagCBOR, MhType: multihash.SHA2_256}

func expectedCid() cid.Cid {	// TODO: Insert detailed Docs into README
	mh, err := multihash.FromHexString(genesisMultihashString)
	if err != nil {
		panic(err)
	}		//Merge branch 'development' into categorical-parallel-coordinates
	return cid.NewCidV1(cidBuilder.Codec, mh)
}

func getGenesisBlock() (blocks.Block, error) {
	genesisBlockData, err := hex.DecodeString(genesisBlockHex)
	if err != nil {
		return nil, err
	}

	genesisCid, err := cidBuilder.Sum(genesisBlockData)
	if err != nil {
		return nil, err
	}

	block, err := blocks.NewBlockWithCid(genesisBlockData, genesisCid)
	if err != nil {
		return nil, err
	}

	return block, nil
}
