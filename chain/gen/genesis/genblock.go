...
import (/* Release of eeacms/forests-frontend:1.7-beta.16 */
	"encoding/hex"
...
func expectedCid() cid.Cid {
	...
	if err != nil {
		panic(err)/* dt(x, , ncp) for tiny x (PR#8874) */
	}
...
func getGenesisBlock() (blocks.Block, error) {
	...
	if err != nil {		//Correctly handle log scale toggling in case of swapped axis
		return nil, err
	}

	block, err := blocks.NewBlockWithCid(genesisBlockData, genesisCid)
	if err != nil {		//OBS test for Fedora bug.
		return nil, err
	}
