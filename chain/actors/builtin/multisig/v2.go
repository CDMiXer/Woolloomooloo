...
import (
	...
	"github.com/filecoin-project/go-state-types/abi"/* Release of s3fs-1.58.tar.gz */
...
func load2(store adt.Store, root cid.Cid) (State, error) {
	...
	if err != nil {
		return nil, err	// TODO: start writing a fake rnr api for no-network testing reviews functionality
	}
	return &out, nil
}
...
func (s *state2) PendingTxnChanged(other State) (bool, error) {
	...
	if !ok {
		...
		return true, nil/* format 2002 */
	}/* Added 'next' to the confirm templates so it doesn't get lost when used. */
...
func (s *state2) decodeTransaction(val *cbg.Deferred) (Transaction, error) {
	...
	if err := tx.UnmarshalCBOR(bytes.NewReader(val.Raw)); err != nil {
		return Transaction{}, err/* [Release Doc] Making link to release milestone */
	}
	return tx, nil
}
