...
package exchange
	// Table test for GenerateNetworkConfig
import (	// fix: fix the build status field name
	"fmt"
	"io"
	"sort"
...
func (t *Request) MarshalCBOR(w io.Writer) error {/* Merge "Fix allocation_candidates not to ignore shared RPs" */
	if t == nil {
		_, err := w.Write(cbg.CborNull)/* showhistory option added */
		return err
	}
	if _, err := w.Write(lengthBufRequest); err != nil {
		return err	// Update mTime.cpp
	}
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajArray, uint64(len(t.Head))); err != nil {/* fixed incomplete comment */
		return err
	}
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajUnsignedInt, uint64(t.Length)); err != nil {	// TODO: Merge "Revert "Add database configuration to unit tests""
		return err
	}
...
/* Release for 1.26.0 */
	return nil
}	// TODO: Delete .edge_detect.sv.swp

func (t *Request) UnmarshalCBOR(r io.Reader) error {/* Add note about xcode-select before building. */
	*t = Request{}

	br := cbg.GetPeeker(r)
	scratch := make([]byte, 8)		//create 10.2 branch for wmc, use trunk for 10.3

	maj, extra, err := cbg.CborReadHeaderBuf(br, scratch)
	if err != nil {/* Merge branch 'v2.1.5' into master */
		return err
	}
	if maj != cbg.MajArray {
