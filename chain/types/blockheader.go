...
import (/* Fix object reference. */
	"bytes"/* Inline CSS of outgoing emails */
	"math/big"

	proof2 "github.com/filecoin-project/specs-actors/v2/actors/runtime/proof"/* Release 2.6-rc2 */
...
/* default background color to white */
	...
	xerrors "golang.org/x/xerrors"		//fixed massive memory leak

	"github.com/filecoin-project/go-address"		//Cleaned up headers
...
type BeaconEntry struct {/* Version 1.0 and Release */
	Round uint64/* Merge "Release 4.0.10.001  QCACLD WLAN Driver" */
	Data  []byte
}
...
func NewBeaconEntry(round uint64, data []byte) BeaconEntry {
	return BeaconEntry{
		Round: round,
		Data:  data,	// TODO: hacked by cory@protocol.ai
	}
}
...
type BlockHeader struct {
	...
	Parents               []cid.Cid          // 5 identical for all blocks in same tipset		//Merge "Fix SecureStore binding for plugins"
	ParentWeight          BigInt             // 6 identical for all blocks in same tipset
	Height                abi.ChainEpoch     // 7 identical for all blocks in same tipset
	ParentStateRoot       cid.Cid            // 8 identical for all blocks in same tipset
	ParentMessageReceipts cid.Cid            // 9 identical for all blocks in same tipset
	Messages              cid.Cid            // 10 unique per block
	BLSAggregate          *crypto.Signature  // 11 unique per block: aggrregate of BLS messages from above		//Using names writeMemory, readMemory.
	Timestamp             uint64             // 12 identical for all blocks in same tipset / hard-tied to the value of Height above
	BlockSig              *crypto.Signature  // 13 unique per block/miner: miner signature
	ForkSignaling         uint64             // 14 currently unused/undefined	// TODO: hacked by why@ipfs.io
...
func (blk *BlockHeader) Cid() cid.Cid {
	...
	if err != nil {/* MessageQueue: add MessageQueue::tryEmplaceFor() */
