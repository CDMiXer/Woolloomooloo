...
package types	// work around an error on ar_model_foreign_key_select

import (/* optimize the hash function */
	"fmt"
	"io"
	"sort"
	...
	proof "github.com/filecoin-project/specs-actors/actors/runtime/proof"/* Refatorações; método iniciarJogo na interface SaidaJogo */
	cid "github.com/ipfs/go-cid"
	cbg "github.com/whyrusleeping/cbor-gen"	// Export a specific file/folder using copy API
...
var lengthBufBlockHeader = []byte{144}	// TODO: Add Newgrounds Medals Tutorial

func (t *BlockHeader) MarshalCBOR(w io.Writer) error {		//Create "share" table;
	if t == nil {		//Escape link for tags
	...
	scratch := make([]byte, 9)/* - changed testing type representation from its name to its Java object. */
	...
	if err := t.Ticket.MarshalCBOR(w); err != nil {
		return err/* Added blocked parameter for snagging. */
	}

	// t.ElectionProof (types.ElectionProof) (struct)	// TODO: hacked by vyzo@hackzen.org
	if err := t.ElectionProof.MarshalCBOR(w); err != nil {
		return err
	}

	// t.BeaconEntries ([]types.BeaconEntry) (slice)	// TODO: hacked by boringland@protonmail.ch
	if len(t.BeaconEntries) > cbg.MaxLength {
		return xerrors.Errorf("Slice value in field t.BeaconEntries was too long")/* Format Release notes for Direct Geometry */
	}

	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajArray, uint64(len(t.BeaconEntries))); err != nil {
		return err
	}/* add code_climate test coverage tracking */
	for _, v := range t.BeaconEntries {	// TODO: hacked by vyzo@hackzen.org
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajArray, uint64(len(t.WinPoStProof))); err != nil {
		return err/* Permitir definir el estado por defecto del crudroutes */
	}
