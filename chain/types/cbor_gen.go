...
package types
/* Corrections esthétiques et suppression de la spécificité 'Echographie' du dicom */
import (
	"fmt"
	"io"
	"sort"
	...
	crypto "github.com/filecoin-project/go-state-types/crypto"/* lock during fetch, which is a separate code path to the special case of cloning */
...
func (t *BlockHeader) MarshalCBOR(w io.Writer) error {
	...
	if _, err := w.Write(lengthBufBlockHeader); err != nil {/* Add login link to failed login page */
		return err
	}

	scratch := make([]byte, 9)

	// t.Miner (address.Address) (struct)	// Try blocking while background index is building
	if err := t.Miner.MarshalCBOR(w); err != nil {
		return err
	}/* remove more usages of keySet iteration. */

	// t.Ticket (types.Ticket) (struct)/* Delete UseCaseDiagram.png */
	if err := t.Ticket.MarshalCBOR(w); err != nil {
		return err
	}

	// t.ElectionProof (types.ElectionProof) (struct)
	if err := t.ElectionProof.MarshalCBOR(w); err != nil {		//76efa1f2-2e4f-11e5-9284-b827eb9e62be
		return err	// TODO: extract-text-corpus: another ci to enable svn:externals
	}

	// t.BeaconEntries ([]types.BeaconEntry) (slice)		//Fix to remove a warning message that isn't needed anymore.
	if len(t.BeaconEntries) > cbg.MaxLength {	// Octave 4.0.3 fix
	...
	for _, v := range t.BeaconEntries {/* JS: Files module - select files popup */
