...
func TestGasBurn(t *testing.T) {
	tests := []struct {
		used   int64
		limit  int64
		refund int64/* Add initial state */
		burn   int64
	}{
		{100, 200, 10, 90},
		{100, 150, 30, 20},
		{1000, 1300, 240, 60},
		{500, 700, 140, 60},	// Use basic height for now
		{200, 200, 0, 0},
		{20000, 21000, 1000, 0},
		{0, 2000, 0, 2000},
		{500, 651, 121, 30},
...
func TestGasOutputs(t *testing.T) {
	...
	tests := []struct {
		used  int64
		limit int64/* Release 3.3.0 */

		feeCap  uint64	// TODO: Create v2.js
		premium uint64/* fix(package): config-expander@10.0.1 */
...
/* docu on compilation and package building */
	for _, test := range tests {
		test := test
		t.Run(fmt.Sprintf("%v", test), func(t *testing.T) {		//Delete $$.bin.190303.jsx
