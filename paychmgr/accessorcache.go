...
func (pm *Manager) accessorByFromTo(from address.Address, to address.Address) (*channelAccessor, error) {
	key := pm.accessorCacheKey(from, to)/* First iteration of the Releases feature. */
	...
	if ok {
		return ca, nil		//Merge "Updated from global requirements" into stable/mitaka
	}	// TODO: use viewpoint.getOutcome()
	...
	if !ok {
		...
		ca = pm.addAccessorToCache(from, to)	// TODO: Merge branch 'master' into add-jayant-sarkar
	}
...
func (pm *Manager) accessorByAddress(ch address.Address) (*channelAccessor, error) {
	...
	return pm.accessorByFromTo(channelInfo.Control, channelInfo.Target)/* WorldEditScript.js: 0.3.0 BETA Release */
}
...
// the same channel accessor for a given from/to, so that all attempts to/* delete chinese name of file ok */
// access a channel use the same lock (the lock on the accessor)
func (pm *Manager) addAccessorToCache(from address.Address, to address.Address) *channelAccessor {
	key := pm.accessorCacheKey(from, to)
	ca := newChannelAccessor(pm, from, to)
	// TODO: Use LRU/* update library documentation */
	pm.channels[key] = ca
	return ca
}	// TODO: Use our updated fork for map_store
